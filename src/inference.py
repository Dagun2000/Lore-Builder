...
def infer_event(resolved_entities: dict, raw_text: str, years: list) -> InferredEvent:
    ...
    prompt = (
        ...
        "서로 무관한 별개의 사건들이 한 문장에 함께 나열된 경우뿐이다(예: '데이비드가 2080년에 "
        ...
        "target 채우기 자체를 생략하지 마라. 예: '[데이비드]가 [알카미아]에 수감되었다'는 "
        ...
        "사건을 직접 행하는 주체는 거의 항상 true다. 예: '[밥]이 [데이비드]의 무덤을 파헤쳤다'에서 "
        "밥은 true, 데이비드는 false다(데이비드는 이미 죽었을 수 있는 과거의 흔적일 뿐 이 사건에 살아서 "
...
def infer_new_entity_attributes(category: str, tag: str, context_sentence: str, years: list) -> dict:
    ...
    prompt = (
        ...
        "데이비드의 가입을 거절했다', '봉인되었다', '술을 먹고 싸우다가', '유리창이 터졌다') — 이건 "
