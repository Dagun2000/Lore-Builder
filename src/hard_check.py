...
def check_terminal_violation(category: str, entity_id: str) -> Conflict | None:
...
def check_lifespan_violation(character_id: str) -> Conflict | None:
...
def run_hard_checks(category: str, entity_id: str) -> list[Conflict]:
    """Run every applicable hard check for an entity and collect the conflicts."""
    conflicts = []

    terminal = check_terminal_violation(category, entity_id)
    if terminal is not None:
        conflicts.append(terminal)

    if category == "character":
        lifespan = check_lifespan_violation(entity_id)
